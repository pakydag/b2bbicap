            ...
            @if((Auth::user()->role === 'agent' || Auth::user()->role === 'admin') && $order->status !== 'confirmed' && $order->status !== 'cancelled')
                <form action="{{ route('agent.orders.update_items', $order) }}" method="POST">
                    ...
                    <div class="p-6 bg-gray-50 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <button type="submit" class="px-8 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black uppercase tracking-wider shadow-lg transition">
                            💾 Salva Modifiche Ordine
                        </button>
                    </div>
                </form>
            @else
                <!-- CLIENTE O ORDINE CONFERMATO/ANNULLATO (Lettura Bloccata) -->
