                    ...
                    <form action="{{ route('admin.b2b.customers.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="b2b_price_list_id" id="b2b_price_list_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach($priceLists as $list)
                                        <option value="{{ $list->id }}" {{ old('b2b_price_list_id') == $list->id ? 'selected' : '' }}>
                            ...
                            <div class="col-span-2 border-t border-gray-200 pt-6 mt-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="mb-4">
                                        <label for="b2b_email" class="block text-sm font-medium text-gray-700">Email di Accesso / Login</label>
                                        <input type="email" name="b2b_email" id="b2b_email" value="{{ old('b2b_email') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('b2b_email') border-red-500 @enderror">
                                        @error('b2b_email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                    </div>
                                    <div class="mb-4">
                                        <label for="b2b_password" class="block text-sm font-medium text-gray-700">Password (lascia vuoto per usare quella di default: Bicap2026!)</label>
