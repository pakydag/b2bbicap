<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
                <h2 class="font-black text-xl text-gray-800 leading-tight uppercase tracking-tight">
                    {{ __('Nuova Condizione di Pagamento') }}
                </h2>
                <p class="text-xs text-gray-500 mt-0.5 font-bold">Creazione termini di pagamento commerciali</p>
            </div>
    ...
    <div class="max-w-xl space-y-6">
        <div class="bg-white shadow-sm rounded-2xl border border-gray-100 p-6">
            <form action="{{ route('admin.b2b.payment-conditions.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="name" class="block text-xs font-black text-gray-700 uppercase tracking-wider mb-1">Titolo (es. Bonifico 30gg) *</label>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" class="block w-full border-gray-200 rounded-xl text-xs font-medium text-slate-800 focus:ring-yellow-400 focus:border-yellow-400 shadow-sm">{{ old('description') }}</textarea>
                </div>
